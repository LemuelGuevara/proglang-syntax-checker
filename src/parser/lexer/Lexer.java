...
public class Lexer {
  ...
  private Token getNumericLiterals() {
  ...
  private Token handleDelimitersAndOperators() {
    switch (currentChar) {
      case '(':
        advance();
  ...
  public Token getNextToken() {
    while (currentChar != '\0') {
      ...
      if (Character.isLetter(currentChar)) {
        Token token = getLiteralsAndIdentifiers();
      ...
      if (Character.isDigit(currentChar))
        return getNumericLiterals();

      // Handle delimiters and operators
      Token token = handleDelimitersAndOperators();
      if (token != null)
        return token;
    }
  ...
  private Token getLiteralsAndIdentifiers() {
