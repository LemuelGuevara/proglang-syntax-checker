...
public class LoopHeaderParser extends Parser {
  ...
  private boolean isCorrectInitialization() {
    ...
    while (currentToken.getType() != TokenType.EOF) {
      ...
      if (currentToken.getType().equals(TokenType.INTEGER_LITERAL) && !foundIdentifier)
        return false;
  ...
  private boolean isCorrectCondition() {
    ...
    while (currentToken.getType() != TokenType.EOF) {
      ...
      if (currentToken.getValue().equals(initializationToken.getValue()) && initializationToken.getValue() != null)
    ...
    return foundIdentifier && foundOperand && matchingInitialization &&
  ...
  private boolean isCorrectIncrementDecrement() {
    if (findSemicolon(2)) {
      ...
      return foundValidIncrement || foundValidDecrement;
    }
    return false;
  }
  ...
  public void runChecks() {
    ...
    if (isCorrectInitialization()) {
      System.out.println("Found correct initialization");

      // Only check condition if initialization is valid
      if (isCorrectCondition()) {
        System.out.println("Found correct condition");
      } else {
    ...
    if (isCorrectIncrementDecrement()) {
      System.out.println("Found correct increment decrement");
    } else {
      System.out.println("Incorrect increment decrement");
    }
  }
}